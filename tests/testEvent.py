...
import pyximport
pyximport.install()
from vlcp_event_cython.event import Event,EventMatcher,withIndices
#from vlcp.event.matchtree import MatchTree, EventTree
from vlcp_event_cython.matchtree import MatchTree, EventTree
#from vlcp.event.pqueue import CBQueue
from vlcp_event_cython.pqueue import CBQueue
...
class Test(unittest.TestCase):
    ...
    def testPickling(self):
        import pickle
        s1 = pickle.dumps(self.event6)
        e2 = pickle.loads(s1)
        self.assertEqual(type(self.event6), type(e2))
        self.assertEqual(self.event6.indices, e2.indices)
        self.assertEqual(self.event6.__dict__, e2.__dict__)
        matcher = TestEvent3.createMatcher('READ', None, 17)
        s1 = pickle.dumps(matcher)
        m2 = pickle.loads(s1)
        self.assertEqual(type(matcher), type(m2))
        self.assertEqual(matcher.indices, m2.indices)
        try:
            import cPickle
        except Exception:
            pass
        else:
            s1 = cPickle.dumps(self.event6)
            e2 = cPickle.loads(s1)
            self.assertEqual(type(self.event6), type(e2))
            self.assertEqual(self.event6.indices, e2.indices)
            self.assertEqual(self.event6.__dict__, e2.__dict__)
            s1 = cPickle.dumps(matcher)
            m2 = cPickle.loads(s1)
            self.assertEqual(type(matcher), type(m2))
            self.assertEqual(matcher.indices, m2.indices)
